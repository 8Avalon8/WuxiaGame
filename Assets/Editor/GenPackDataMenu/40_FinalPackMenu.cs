...
namespace JianghuX.Editor
{
    ...
    public class FinalPackMenuCmd
    {
        ...
        [MenuItem("Tools♥/[打包ANDROID简体]")]
        static void Pack4Android()
        {
            MenuHelper.SafeWrapMenuAction("打包ANDROID简体", title => PackAllDataInner(false, title, false, null));
        }
        ...
        static void PackAllDataInner(bool isBuildForIOS, string title, bool force, string language)
        {
            using (HSUtils.ExeTimer("PackAllDataInner {0}".f(title)))
            {
                ...
                try
                {
                    GenDataMenuCmd.GenPackDataAllTheWay(title, force); //支持启动前和运行中取消
