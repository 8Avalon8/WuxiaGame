...
namespace HSUI
{
    ...
    public class DefaultUIManager : IUIManager
    {
        ...
        public void Init(Transform rootGameObject, IUIProvider provider = null)
        {
            if(provider == null)
            {
                _provider = new ResourcePrefabLeanPoolProvider();
            }
            else
            {
                _provider = provider;
        ...
        /// <param name="paras"></param>
        /// <returns></returns>
        public T CreatePanel<T>(string path,Transform transform, params object[] paras) where T : MonoBehaviour, IHSUIComponent
        {
            T panel = _provider.Spawn<T>(path);
        ...
        public bool DestroyPanel(IHSUIComponent panel)
        {
            ...
            if(panel == _current.node)
            {
                while(true)
                {
                    ...
                    {
                        _current = link;
                        ShowPanel(link.node);//by an:销毁后显示上一个panel
